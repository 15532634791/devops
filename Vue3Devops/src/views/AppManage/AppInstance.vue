<template>
    ...
    <div class="container" style=" max-width:100%">
        ...
        <el-table :data="currentPageData" stripe style="width: 100%; margin-top: 5px">
            <el-table-column fixed="left" prop="Name" label="应用名称" width="300"/>
            <el-table-column prop="AppId" label="AppId" width="320"/>
            <el-table-column prop="ClusterId" label="集群Id" width="320"/>
            <el-table-column prop="RegionId" label="命名空间" width="250"/>
            <el-table-column prop="ClusterType" label="集群类型" width="150">
              <template #default="scope">
                <template v-if="scope.row.ClusterType === 5">
                  <span>容器服务K8S集群</span>
                </template>
                <template v-else-if="scope.row.ClusterType === 2">
                  <span>ECS集群</span>
                </template>
              </template>
            </el-table-column>
            <el-table-column prop="RunningInstanceCount" label="运行中的实例数" width="130"/>
            <el-table-column prop="InstanceCount" label="实例总数" width="100"/>
...
onBeforeMount(() => {
    ...
    axios.get('/api/project/manage/app/list').then(response => {
...
const syncImageData = () => {
    ...
    axios.get('http://127.0.0.1:8001/project/manage/app/refresh/').then(response => {
        console.log(response.data);
...
const filteredData = computed(() => {
    if (searchText.value === '') {
        ...
        return dataList.value.filter(item => item.Name.includes(searchText.value));
    }
});
